...
class ImageExtensionTest: XCTestCase {
    ...
    func testBackgroundImageSun_ReturnEqual() {
        ...
        let image = viewController.setIconForecastWeather(currentCondition: weatherViewModel.objectCurrentWeather?.weather?[0].main ?? "")
        XCTAssertEqual(image, UIImage.sunnyIcon)
    }
    
    func testBackgroundImageSun_ReturnNotEqual() {
        mockWeatherRepository.shouldPass = false
        mockWeatherRepository.weatherCondition = "sunny"
        weatherViewModel.fetchCurrentWeatherResults { _ in
        }
        let image = viewController.setIconForecastWeather(currentCondition: weatherViewModel.objectCurrentWeather?.weather?[0].main ?? "")
        XCTAssertNotEqual(image, UIImage.rainyIcon)
    }
    
    func testBackgroundImageClear_ReturnEqual() {
        mockWeatherRepository.shouldPass = true
    ...
    func testBackgroundImageClear_ReturnNotEqual() {
        mockWeatherRepository.shouldPass = false
        mockWeatherRepository.weatherCondition = "clear"
        weatherViewModel.fetchCurrentWeatherResults { _ in
        }
    ...
    func testBackgroundImageRain_ReturnEqual() {
        mockWeatherRepository.weatherCondition = "rain"
        mockWeatherRepository.shouldPass = true
        weatherViewModel.fetchCurrentWeatherResults { _ in
        }
        let image = viewController.setIconForecastWeather(currentCondition: weatherViewModel.objectCurrentWeather?.weather?[0].main ?? "")
        XCTAssertEqual(image, UIImage.rainyIcon)
    }
    
    func testBackgroundImageRain_ReturnNotEqual() {
        mockWeatherRepository.weatherCondition = "rain"
        mockWeatherRepository.shouldPass = false
        weatherViewModel.fetchCurrentWeatherResults { _ in
        }
        let image = viewController.setIconForecastWeather(currentCondition: weatherViewModel.objectCurrentWeather?.weather?[0].main ?? "")
        XCTAssertNotEqual(image, UIImage.cloudyIcon)
    }
    
    func testBackgroundImageCloud_ReturnEqual() {
        mockWeatherRepository.weatherCondition = "clouds"
        mockWeatherRepository.shouldPass = true
        weatherViewModel.fetchCurrentWeatherResults { _ in
        }
        let image = viewController.setIconForecastWeather(currentCondition: weatherViewModel.objectCurrentWeather?.weather?[0].main ?? "")
        XCTAssertEqual(image, UIImage.cloudyIcon)
    }
    
    func testBackgroundImageCloud_ReturnNotEqual() {
        mockWeatherRepository.weatherCondition = "clouds"
        mockWeatherRepository.shouldPass = false
        weatherViewModel.fetchCurrentWeatherResults { _ in
        }
